...
## Runtime Execution (JVM/Kotlin Projects)

When working in a JVM/Kotlin project, **delegate all test execution to
`kotlin-jvm-lsp-gradle-debug`**. Do not run Gradle commands ad-hoc; that skill owns the
build/test/debug loop.

Handoff points:
- **Before the tracer bullet** — invoke `kotlin-jvm-lsp-gradle-debug` Phase 0 pre-flight
  to confirm the environment is healthy before writing any code.
- **After each RED→GREEN cycle** — invoke `kotlin-jvm-lsp-gradle-debug` to run the test
  and read results from JUnit XML (never from console scroll).
- **On any unexpected failure** — follow the recovery sequence in
  `kotlin-jvm-lsp-gradle-debug` before diagnosing test logic.

For non-JVM projects, run whatever test command is appropriate for the stack; the same
one-test-at-a-time discipline applies.
